...
def get_loader(args):
    ...
    train_sampler = RandomSampler(trainset[:100]) if args.local_rank == -1 else DistributedSampler(trainset)
    ...
    train_loader = DataLoader(trainset,
                              ...
                              num_workers=4,
                              drop_last=True,
                              pin_memory=True) #🌟bs: 16
    test_loader = DataLoader(testset,
                             sampler=test_sampler,
                             batch_size=args.eval_batch_size, 
                             num_workers=4,
                             pin_memory=True) if testset is not None else None #🌟bs: 8
